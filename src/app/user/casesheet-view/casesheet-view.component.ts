...
import { UHID, UserInfo, OCUserInfo, appointmentDetails } from '@aa/structures/user.interface';
...
@Component({
  selector: 'app-casesheet-view',
  templateUrl: './casesheet-view.component.html',
  styleUrls: ['./casesheet-view.component.scss']
})
export class CasesheetViewComponent implements OnInit {
  ...
  getPendignCaseSheetInfo() {
    ...
    this.getReports();
    this.aaa.loadingShow('loadingid');
    this.aaa.getNewPendingCaseSheetAllDetails(this.appointmentDetails.VisitId).subscribe(t => {
      this.aaa.loadingHide('loadingid');
      if (t.ResponceCode == "0" && t.Result != undefined && t.Result != null && t.Result != "" && t.Result != "[]") {
        ...
        if (cInfo.length > 0) {
          ...
          if (this.AppointmentPatientInfo.type === 'Relative') {
            for (let a of this.availableRelatives) {
              if (a.patRelationId == pIngo.RelationId) {
                this.selectedRelative = a;
                this.AppointmentPatientInfo.firstName = a.firstName;
                this.AppointmentPatientInfo.lastName = a.lastName;
                this.AppointmentPatientInfo.dob = a.dob;
                this.AppointmentPatientInfo.gender = this.gender.transform(a.gender);
              }
            }
          }
  ...
  ngOnInit() {
    ...
    this.appDetailsSubscription = this.userService.currentAppointment.subscribe(
      (data) => {
    ...
    let visitId = this.appointmentDetails.VisitId;
  ...
  */
  ...
  getReports() {
    ...
    this.userService.getAttachmentss(this.appointmentDetails.VisitId).subscribe(
      (data: any) => {
        if (data != null) {
          if (data.Result != undefined && data.Result != null) {
            let res = JSON.parse(data.Result);
